...
- Estructura de pruebas unitarias, cubriendo motor de cálculo existente — **✅ hecho para Domain** (`Tax`, `ExchangeRate`, `Product`, `ProductCodeCounter`) y arrancado para Identity (`EventLog`). Primer `Application.Tests` del repo creado (Inventory, `GenerateProductCodeCommandHandler`, con `NSubstitute`). **Política confirmada (2026-08-05):** `Application.Tests` es obligatorio para todo handler *nuevo* de acá en adelante, no retroactivo — los ~40+ handlers ya existentes en Identity/MasterData/Inventory sin test quedan como deuda técnica conocida, no bloqueante.
